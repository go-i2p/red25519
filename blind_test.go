...
// TestBlindPublicKeySmallOrderRejection verifies that BlindPublicKey rejects
// small-order input points (not just the identity). Blinding a small-order
// point yields another small-order point, which Verify would reject. Failing
// early in BlindPublicKey gives callers a clear error.
func TestBlindPublicKeySmallOrderRejection(t *testing.T) {
	bf, _ := GenerateBlindingFactor(rand.Reader)
	...
	for _, tc := range smallOrderPoints {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := BlindPublicKey(pub, bf)
			if err == nil {
				t.Errorf("BlindPublicKey should reject small-order point %s", tc.name)
			}
		})
	}
}
